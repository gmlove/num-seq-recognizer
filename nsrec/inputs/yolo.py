...
def preprocess_image(image, main_bbox, bboxes, final_size, is_training):
  ...
  if is_training:
    ...
    with ops.name_scope(None, 'distorted_bounding_box'):
    ...
    with ops.name_scope(None, 'calculate_bounding_box'):
      ...
      bboxes = crop_to_ltwh_bboxes(image, bboxes, xmin, ymin, w, h)

    image_with_box = tf.image.draw_bounding_boxes(
      tf.expand_dims(image, 0), to_yx_yx_bboxes(image, bboxes))
    tf.summary.image('bbox_distorted_images_with_boxes', image_with_box)

  with ops.name_scope(None, 'transform_bounding_box'):
...
def crop_to_ltwh_bboxes(image, bboxes, xmin, ymin, w, h):
  with ops.name_scope(None, 'crop_to_ltwh_bboxes'):
    bboxes = tf.squeeze(bboxes, axis=0)
    image_shape = tf.cast(tf.shape(image), tf.float32)
    to_concat = []

    def crop_to_ltwh_bbox(bbox):
      with ops.name_scope(None, 'crop_to_ltwh_bbox'):
        return tf.concat(
    ...
    for i in range(0, bboxes.get_shape().as_list()[0]):
      bbox = tf.cond(bboxes[i, 3] < 1e-5, lambda: bboxes[i], lambda: crop_to_ltwh_bbox(bboxes[i]))
      to_concat.append(tf.expand_dims(bbox, 0))

  return tf.concat(to_concat, 0)


def to_yx_yx_bboxes(image, bboxes):
  with ops.name_scope(None, 'to_yx_yx_bboxes'):
    image_shape = tf.cast(tf.shape(image), tf.float32)
    bboxes = tf.concat(
...
def batches(data_file_path, max_number_length, batch_size, size,
  ...
  with ops.name_scope(None, 'read-data'):
...
def prepare_for_loss(max_number_length, batch_size, bboxes_batch, image_shape_batch, label_batch):
  with ops.name_scope(None, 'prepare-for-loss'):
    ...
    with ops.name_scope(None, 'init-local-variables'):
    ...
    with ops.name_scope(None, 'calculate-local-variables'):
    ...
    with ops.name_scope(None, 'calculate-final-loss-feed'):
